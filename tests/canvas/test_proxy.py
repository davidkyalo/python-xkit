...
from zana.canvas.proxy import CachingProxy, Proxy, unproxy
...
@pytest.mark.parametrize(
    ...
    [
        (_object, _object),
        (Proxy(lambda: _object), _object),
        (CachingProxy(lambda: _object), _object),
    ],
)
def test_unproxy(obj, val):
    assert unproxy(obj) is val


class test_Proxy:
    def test_std_class_attributes(self):
        assert Proxy.__name__ == "Proxy"
        assert Proxy.__module__ == "zana.canvas.proxy"
    ...
    def test_get_current_local(self):
        mk = Mock()
        x: Proxy = Proxy(mk)
        assert x.__get_proxy_target__() is mk.return_value
...
class test_CachingProxy:
    def test_only_evaluated_once(self):
        ...
        p: CachingProxy = CachingProxy(X)
        assert p.__get_proxy_target__() is p.__get_proxy_target__()
        assert p.attr == 123 == p.attr
        assert p.__class__ is X
        assert X.evals == 1 == p.evals
        old = p.__get_proxy_target__()
        p.__proxy_reset_cache__()
        new = p.__get_proxy_target__()
        assert old is not new
        assert p.__get_proxy_target__() is p.__get_proxy_target__()
        assert X.evals == 2 == p.evals
